...
const links = [
  ...
  ['/admin/verifications', 'ownerVerifications', 'owners', ShieldCheck],
  ['/admin/properties', 'propertyReviews', 'properties', Building2],
  ['/admin/reports', 'safetyReports', 'reports', TriangleAlert],
]
    ...
    <div className="min-h-screen bg-[#070b14] pb-20 text-white lg:pb-0">
      ...
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-72 border-r border-white/[0.07] bg-[#090e19]/95 p-5 backdrop-blur-2xl lg:block">
        <Link to="/" className="flex items-center gap-3 px-3 py-4">
          ...
          <div><p className="font-black tracking-[-.03em]">{t('common.brand')}</p><p className="text-[10px] font-bold uppercase tracking-[.17em] text-violet-300/60">{t('admin.administration')}</p></div>
        </Link>

        <div className="mx-3 mt-4 rounded-[24px] border border-violet-400/15 bg-violet-500/[0.08] p-4">
          <p className="text-[10px] font-black uppercase tracking-[.18em] text-violet-300">{t('admin.controlCentre')}</p>
          <p className="mt-2 text-sm font-black">{t('admin.trustTitle')}</p>
          <p className="mt-1 text-xs leading-5 text-slate-500">{t('admin.trustText')}</p>
        </div>
        ...
        <Link to="/dashboard" className="absolute bottom-6 left-5 right-5 flex items-center justify-center gap-2 rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-3 text-sm font-bold text-slate-400 transition hover:bg-white/[0.06] hover:text-white"><ChevronLeft className="h-4 w-4" /> {t('admin.backToAccount')}</Link>
      </aside>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-50 grid grid-cols-4 border-t border-white/[0.08] bg-[#090e19]/96 px-2 py-2 backdrop-blur-2xl lg:hidden" aria-label={t('admin.mobileNav')}>
        {links.map(([to, , shortKey, Icon]) => (
          <NavLink key={to} to={to} end={to === '/admin'} className={({ isActive }) => `flex min-w-0 flex-col items-center gap-1 rounded-xl px-1 py-2 text-[9px] font-bold transition ${isActive ? 'bg-violet-400/10 text-violet-200' : 'text-slate-500'}`}>
            ...
            <span className="max-w-full truncate">{t(`admin.${shortKey}`)}</span>
          </NavLink>
        ))}
      </nav>
    </div>
