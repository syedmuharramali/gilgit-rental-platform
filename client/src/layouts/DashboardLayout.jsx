...
function DashboardLayout({ mode = 'renter' }) {
  ...
  const isMoreActive = links.slice(4).some(([to]) => location.pathname === to || location.pathname.startsWith(`${to}/`))
  ...
  return (
    <div className="min-h-screen bg-[#070b14] pb-20 text-white lg:pb-0">
      ...
      <nav className="fixed inset-x-0 bottom-0 z-50 grid grid-cols-5 border-t border-white/[0.07] bg-[#090f1b]/95 px-2 py-2 backdrop-blur-2xl lg:hidden" aria-label={`${workspaceLabel} mobile navigation`}>
        {primaryMobileLinks.map(([to, label, Icon]) => <NavLink key={to} to={to} end={to === '/dashboard' || to === '/owner'} className={({ isActive }) => `flex min-w-0 flex-col items-center gap-1 rounded-xl px-1 py-2 text-[9px] font-bold transition ${isActive ? 'bg-cyan-300/10 text-cyan-200' : 'text-slate-500'}`}><Icon className="h-4 w-4" /><span className="max-w-full truncate">{label.split(' ')[0]}</span></NavLink>)}
        <button type="button" onClick={() => setMobileMenuOpen(true)} aria-expanded={mobileMenuOpen} aria-controls="mobile-workspace-menu" className={`flex min-w-0 flex-col items-center gap-1 rounded-xl px-1 py-2 text-[9px] font-bold transition ${isMoreActive ? 'bg-cyan-300/10 text-cyan-200' : 'text-slate-500'}`}><Menu className="h-4 w-4" />More</button>
      </nav>
          ...
          <>
            ...
            <motion.aside id="mobile-workspace-menu" role="dialog" aria-modal="true" aria-label={`${workspaceLabel} menu`} initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} transition={{ type: 'spring', stiffness: 320, damping: 32 }} className="fixed inset-x-0 bottom-0 z-[70] max-h-[82vh] overflow-y-auto rounded-t-[30px] border-t border-white/10 bg-[#0a101c] p-5 pb-8 shadow-[0_-30px_90px_rgba(0,0,0,.45)] lg:hidden">
              ...
              <nav className="grid gap-2 sm:grid-cols-2">
                {links.map(([to, label, Icon]) => <NavLink key={to} to={to} end={to === '/dashboard' || to === '/owner'} onClick={() => setMobileMenuOpen(false)} className={({ isActive }) => `flex min-h-12 items-center gap-3 rounded-2xl border px-4 py-3 text-sm font-bold transition ${isActive ? 'border-cyan-300/20 bg-cyan-300/[0.08] text-cyan-100' : 'border-white/[0.07] bg-white/[0.025] text-slate-400'}`}><Icon className="h-4 w-4 shrink-0" /><span className="min-w-0 flex-1 truncate">{label}</span>{label === 'Notifications' && unread?.unreadCount > 0 && <span className="rounded-full bg-rose-500 px-2 py-0.5 text-[10px] text-white">{unread.unreadCount}</span>}</NavLink>)}
