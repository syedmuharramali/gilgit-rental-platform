...
}, { message: 'prefs.rentError', path: ['maxRent'] })
...
function PreferenceForm({ preferences, amenities = [], onSave, isSaving }) {
  ...
  return (
    <form onSubmit={handleSubmit(submit)} className="space-y-7" noValidate>
      <div>
        <p className="text-sm font-black text-white">{t('prefs.budget')}</p>
        <div className="mt-3 grid gap-3 sm:grid-cols-2">
          <label className="space-y-2"><span className="text-xs font-bold text-white/38">{t('prefs.minimum')}</span><input {...register('minRent')} type="number" min="0" inputMode="numeric" className={inputClass} placeholder="15000" /></label>
          <label className="space-y-2"><span className="text-xs font-bold text-white/38">{t('prefs.maximum')}</span><input {...register('maxRent')} type="number" min="0" inputMode="numeric" className={inputClass} placeholder="35000" /></label>
        </div>
        {errors.maxRent ? <p className="mt-2 text-xs font-bold text-rose-300">{t(errors.maxRent.message, { defaultValue: errors.maxRent.message })}</p> : null}
      </div>

      <Controller name="propertyTypes" control={control} render={({ field }) => (
        <div><p className="text-sm font-black text-white">{t('prefs.propertyTypes')}</p><div className="mt-3 flex flex-wrap gap-2">{propertyTypes.map((value) => { const active = field.value.includes(value); return <button key={value} type="button" aria-pressed={active} onClick={() => field.onChange(active ? field.value.filter((item) => item !== value) : [...field.value, value])} className={`rounded-full border px-3.5 py-2 text-xs font-black transition ${active ? 'border-cyan-300/30 bg-cyan-300/12 text-cyan-100' : 'border-white/9 bg-white/[0.035] text-white/42 hover:border-white/18 hover:text-white/70'}`}>{pretty(value)}</button> })}</div></div>
      )} />

      <div>
        <label className="space-y-2"><span className="text-sm font-black text-white">{t('prefs.preferredAreas')}</span><input {...register('preferredAreas')} className={`mt-3 ${inputClass}`} placeholder={t('prefs.areasPlaceholder')} /></label>
        <p className="mt-2 text-xs text-white/28">{t('prefs.areasHint')}</p>
      </div>

      <div>
        <label className="space-y-2"><span className="text-sm font-black text-white">{t('prefs.minimumBedrooms')}</span><input {...register('minimumBedrooms')} type="number" min="0" max="20" inputMode="numeric" className={`mt-3 ${inputClass}`} placeholder="1" /></label>
      </div>

      <Controller name="furnishedStatuses" control={control} render={({ field }) => (
        <div><p className="text-sm font-black text-white">{t('prefs.furnishing')}</p><div className="mt-3 flex flex-wrap gap-2">{furnishing.map((value) => { const active = field.value.includes(value); return <button key={value} type="button" aria-pressed={active} onClick={() => field.onChange(active ? field.value.filter((item) => item !== value) : [...field.value, value])} className={`rounded-full border px-3.5 py-2 text-xs font-black transition ${active ? 'border-violet-300/30 bg-violet-300/10 text-violet-100' : 'border-white/9 bg-white/[0.035] text-white/42 hover:text-white/70'}`}>{pretty(value)}</button> })}</div></div>
      )} />

      <Controller name="amenities" control={control} render={({ field }) => (
        <div><p className="text-sm font-black text-white">{t('prefs.amenitiesThatMatter')}</p><div className="mt-3 grid gap-2 sm:grid-cols-2">{amenities.map((amenity) => { const id = amenity._id; const active = field.value.includes(id); return <label key={id} className={`flex cursor-pointer items-center gap-3 rounded-2xl border px-4 py-3 text-sm font-bold transition ${active ? 'border-cyan-300/25 bg-cyan-300/8 text-cyan-100' : 'border-white/8 bg-white/[0.025] text-white/42 hover:bg-white/[0.045]'}`}><input type="checkbox" checked={active} onChange={() => field.onChange(active ? field.value.filter((item) => item !== id) : [...field.value, id])} className="accent-cyan-300" />{amenityLabel(amenity)}</label> })}</div></div>
      )} />
