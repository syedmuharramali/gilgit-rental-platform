          ...
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden border-t border-white/8 bg-[#080c18] lg:hidden">
            <div className="space-y-2 px-5 py-5 sm:px-8">
              ...
              <div className="grid grid-cols-2 gap-2 pt-2">
                ...
                {!isAuthenticated && <Link to="/register" onClick={() => setOpen(false)} className="rounded-2xl bg-gradient-to-r from-cyan-300 to-blue-500 px-4 py-3 text-center text-sm font-black text-[#07101e]">{t('nav.joinFree')}</Link>}
              </div>
            </div>
          </motion.div>
        )}
