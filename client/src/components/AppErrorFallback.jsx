  ...
  return (
    <main className="grid min-h-screen place-items-center bg-[#f6f8f7] px-5">
      <div className="w-full max-w-lg rounded-[32px] border border-slate-200 bg-white p-8 text-center shadow-[0_28px_80px_rgba(15,23,42,.09)]">
        ...
        <h1 className="mt-5 text-2xl font-black tracking-[-.04em] text-slate-950">{t('error.title')}</h1>
        <p className="mt-3 text-sm leading-6 text-slate-500">{t('error.text')}</p>
        ...
        <button onClick={resetErrorBoundary} className="mt-6 inline-flex min-h-11 items-center gap-2 rounded-2xl bg-[#102f26] px-5 text-sm font-black text-white"><RotateCcw className="h-4 w-4" /> {t('common.tryAgain')}</button>
      </div>
    </main>
  )
}
