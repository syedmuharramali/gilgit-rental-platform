...
export function AdminReportsPage() {
  ...
  if (isLoading) return <LoadingState />
  const setReport = async (id, nextStatus) => { try { await update({ id, status: nextStatus, adminNotes: notes[id] || '' }).unwrap(); toast.success(t('admin.reportUpdated')) } catch (error) { toast.error(errorMessage(error)) } }
