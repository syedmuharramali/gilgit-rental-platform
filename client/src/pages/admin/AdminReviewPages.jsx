          ...
          <motion.div key={property._id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * .035 }}>
            <Panel className="hover:border-cyan-300/15">
              <div className="grid gap-5 lg:grid-cols-[140px_1fr_auto] lg:items-center">
                ...
                <div><div className="flex flex-wrap gap-2"><StatusBadge value={property.listingStatus} /><span className="text-xs font-bold text-slate-500">{pretty(property.propertyType)}</span></div><h2 className="mt-2 text-lg font-black text-white">{property.title}</h2><p className="mt-1 text-sm text-slate-400">{property.owner?.name} · {property.address?.area} · {(isStayType(property.propertyType) ? t('card.perNightFrom', { price: money(property.nightlyPriceFrom) }) : money(property.monthlyRent))}</p><p className="mt-2 line-clamp-2 text-sm leading-6 text-slate-400">{property.description}</p></div>
    ...
    <>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.25fr_.75fr]">
        <div className="space-y-6">
          ...
          <Panel><div className="flex flex-wrap gap-2"><StatusBadge value={property.listingStatus} /><span className="rounded-full border border-white/8 bg-white/[0.04] px-3 py-1 text-xs font-black text-slate-300">{pretty(property.propertyType)}</span></div><h2 className="mt-5 text-xl font-black text-white">{t('adm.listingDetails')}</h2><p className="mt-3 whitespace-pre-line text-sm leading-7 text-slate-400">{property.description}</p><div className="mt-5 grid gap-3 sm:grid-cols-3">{(isStayType(property.propertyType)
            ? [[t('ed.room.fromPrice'),t('card.perNightFrom', { price: money(property.nightlyPriceFrom) })],[t('ed.step.rooms'),(property.roomTypes || []).map((room) => `${room.quantity} × ${room.name} (${money(room.nightlyPrice)})`).join(', ') || '—'],[t('ed.field.checkInTime'),`${property.checkInTime || '14:00'} / ${property.checkOutTime || '12:00'}`]]
            : property.propertyType === 'shop'
            ? [[t('adm.monthlyRent'),money(property.monthlyRent)],[t('adm.security'),money(property.securityDeposit)],[t('ed.field.totalArea'),property.totalArea?.value ? `${property.totalArea.value} ${pretty(property.totalArea.unit || 'sqft')}` : '—'],[t('ed.field.washrooms'),property.bathrooms]]
            : [[t('adm.monthlyRent'),money(property.monthlyRent)],[t('adm.security'),money(property.securityDeposit)],[t('adm.furnishing'),pretty(property.furnishedStatus)],[t('adm.bedrooms'),property.bedrooms],[t('adm.bathrooms'),property.bathrooms],[t('adm.occupants'),property.maxOccupants]]).map(([label,value]) => <div key={label} className={`${softCard} p-4`}><p className="text-[10px] font-bold text-slate-500">{label}</p><p className="mt-1 text-sm font-black text-white">{value}</p></div>)}</div></Panel>
