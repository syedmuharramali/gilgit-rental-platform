          ...
          <motion.div key={property._id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * .035 }}>
            <Panel className="hover:border-cyan-300/15">
              <div className="grid gap-5 lg:grid-cols-[140px_1fr_auto] lg:items-center">
                ...
                <Link to={`/admin/properties/${property._id}`} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-cyan-300 via-blue-400 to-violet-500 px-4 text-sm font-black text-[#07101e]">Review <ExternalLink className="h-4 w-4" /></Link>
              </div>
            </Panel>
          </motion.div>
    ...
    <>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.25fr_.75fr]">
        <div className="space-y-6">
          <Panel className="p-3"><div className="grid gap-3 sm:grid-cols-2">{property.images?.length ? property.images.map((image) => <div key={image.id} className="relative overflow-hidden rounded-2xl ring-1 ring-white/10"><img src={image.url} alt={image.alt || property.title} className="aspect-[4/3] h-full w-full object-cover" />{image.isCover && <span className="absolute left-3 top-3 rounded-full bg-gradient-to-r from-cyan-300 to-blue-400 px-3 py-1 text-[10px] font-black text-[#07101e]">Cover</span>}</div>) : <div className="col-span-2 grid min-h-60 place-items-center rounded-2xl bg-white/[0.035] text-sm font-bold text-slate-500">No images</div>}</div></Panel>
          <Panel><div className="flex flex-wrap gap-2"><StatusBadge value={property.listingStatus} /><span className="rounded-full border border-white/8 bg-white/[0.04] px-3 py-1 text-xs font-black text-slate-300">{pretty(property.propertyType)}</span></div><h2 className="mt-5 text-xl font-black text-white">Listing details</h2><p className="mt-3 whitespace-pre-line text-sm leading-7 text-slate-400">{property.description}</p><div className="mt-5 grid gap-3 sm:grid-cols-3">{[['Monthly rent',money(property.monthlyRent)],['Security',money(property.securityDeposit)],['Furnishing',pretty(property.furnishedStatus)],['Bedrooms',property.bedrooms],['Bathrooms',property.bathrooms],['Occupants',property.maxOccupants]].map(([label,value]) => <div key={label} className={`${softCard} p-4`}><p className="text-[10px] font-bold text-slate-500">{label}</p><p className="mt-1 text-sm font-black text-white">{value}</p></div>)}</div></Panel>
          <Panel><h2 className="font-black text-white">Amenities & Gilgit living data</h2><div className="mt-4 flex flex-wrap gap-2">{property.amenities?.map((amenity) => <span key={amenity._id} className="rounded-full border border-cyan-300/10 bg-cyan-300/[0.06] px-3 py-2 text-xs font-bold text-cyan-100">{amenity.name}</span>)}</div><div className="mt-5 grid gap-3 sm:grid-cols-2">{[['Heating',property.livingInfo?.heatingAvailable],['Hot water',property.livingInfo?.hotWaterAvailable],['Power backup',property.livingInfo?.electricityBackup],['Winter access',property.livingInfo?.winterAccessible],['Water',pretty(property.livingInfo?.waterAvailability)],['Road',pretty(property.livingInfo?.roadAccess)]].map(([label,value]) => <div key={label} className={`${softCard} p-3 text-sm`}><span className="font-bold text-slate-400">{label}</span><strong className="float-right text-white">{typeof value === 'boolean' ? (value ? 'Yes' : 'No') : value}</strong></div>)}</div></Panel>
        </div>
        <aside className="space-y-5"><Panel className="sticky top-8"><div className="grid h-12 w-12 place-items-center rounded-2xl bg-cyan-300/10 text-cyan-200 ring-1 ring-cyan-300/15"><ShieldCheck className="h-6 w-6" /></div><h2 className="mt-4 text-lg font-black text-white">Moderation decision</h2><p className="mt-2 text-sm leading-6 text-slate-400">Submitted {shortDate(property.submittedAt)}. Confirm the listing is coherent and suitable for publication.</p>{property.rejectionReason && <p className="mt-4 rounded-2xl border border-rose-400/15 bg-rose-400/[0.06] p-3 text-sm text-rose-300">Previous rejection: {property.rejectionReason}</p>}{property.listingStatus === 'pending_review' && <div className="mt-5 space-y-2"><PrimaryButton disabled={approveState.isLoading} className="w-full" onClick={() => decide('approve')}>Approve & publish</PrimaryButton><SecondaryButton className="w-full" onClick={() => setRejectOpen(true)}>Reject with reason</SecondaryButton></div>}</Panel></aside>
      </div>
...
export function AdminVerificationsQueuePage() {
  ...
  return (
    <>
      ...
      <div className="space-y-4">{requests.length ? requests.map((verification, index) => <motion.div key={verification._id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * .035 }}><Panel><div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between"><div><StatusBadge value={verification.status} /><h2 className="mt-2 font-black text-white">{verification.user?.name}</h2><p className="mt-1 text-sm text-slate-400">{verification.user?.email} · CNIC ending {verification.cnicLast4} · attempt #{verification.attemptNumber}</p><p className="mt-2 text-xs text-slate-500">Submitted {shortDate(verification.submittedAt)}</p></div><Link to={`/admin/verifications/${verification._id}`} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-cyan-300 via-blue-400 to-violet-500 px-4 text-sm font-black text-[#07101e]">Inspect documents <ExternalLink className="h-4 w-4" /></Link></div></Panel></motion.div>) : <EmptyState title="No verification requests" />}</div>
    </>
  )
}
...
export function AdminVerificationReviewPage() {
  ...
  return (
    <>
      <Link to="/admin/verifications" className="mb-5 inline-flex items-center gap-2 text-sm font-black text-slate-400 transition hover:text-cyan-300"><ArrowLeft className="h-4 w-4" /> Verification queue</Link>
      ...
      <div className="grid gap-6 xl:grid-cols-[.8fr_1.2fr]">
        <Panel><StatusBadge value={verification.status} /><h2 className="mt-5 font-black text-white">Submission details</h2><div className="mt-4 space-y-3 text-sm">{[['Attempt',`#${verification.attemptNumber}`],['Submitted',shortDate(verification.submittedAt)],['Account',verification.user?.accountStatus],['Phone',verification.user?.phone || 'Not provided']].map(([label,value]) => <div key={label} className={`flex justify-between gap-4 ${softCard} p-3`}><span className="text-slate-400">{label}</span><strong className="text-white">{value}</strong></div>)}</div>{verification.rejectionReason && <p className="mt-4 rounded-2xl border border-rose-400/15 bg-rose-400/[0.06] p-4 text-sm text-rose-300">{verification.rejectionReason}</p>}{verification.status === 'pending' && <div className="mt-5 space-y-2"><PrimaryButton disabled={approveState.isLoading} className="w-full" onClick={() => decide('approve')}>Approve identity</PrimaryButton><SecondaryButton className="w-full" onClick={() => setRejectOpen(true)}>Reject / request resubmission</SecondaryButton></div>}</Panel>
        <Panel><div className="flex items-center gap-3"><div className="grid h-10 w-10 place-items-center rounded-2xl bg-violet-400/10 text-violet-200 ring-1 ring-violet-300/15"><FileImage className="h-5 w-5" /></div><h2 className="font-black text-white">Private documents</h2></div><p className="mt-2 text-xs leading-5 text-slate-500">These files are fetched only through the protected administrator endpoint and are not public URLs.</p><div className="mt-5 grid gap-3 sm:grid-cols-3">{[['cnicFront','CNIC front'],['cnicBack','CNIC back'],['selfie','Selfie']].map(([type,label]) => <button key={type} onClick={() => viewDocument(type,label)} className="rounded-2xl border border-white/10 bg-white/[0.035] p-4 text-left text-sm font-black text-slate-200 transition hover:border-cyan-300/25 hover:bg-cyan-300/[0.05]">{label}<span className="mt-1 block text-[10px] font-semibold text-slate-500">Open securely</span></button>)}</div>{documentUrl ? <div className="mt-5 overflow-hidden rounded-[24px] border border-white/10 bg-[#080d17]"><div className="flex items-center justify-between border-b border-white/10 bg-[#070b14] px-4 py-3 text-white"><span className="text-xs font-black">{documentLabel}</span><a href={documentUrl} target="_blank" rel="noreferrer" className="text-xs font-black text-cyan-300">Open full size</a></div><img src={documentUrl} alt={documentLabel} className="max-h-[620px] w-full object-contain" /></div> : <div className="mt-5 grid min-h-72 place-items-center rounded-[24px] border border-dashed border-white/10 bg-white/[0.025] text-sm font-bold text-slate-500">Select a document to inspect</div>}</Panel>
      </div>
      <Modal open={rejectOpen} onClose={() => setRejectOpen(false)} title="Verification decision"><TextArea value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Explain why the identity submission cannot be approved" /><label className="mt-4 flex items-center gap-3 rounded-2xl border border-white/8 bg-white/[0.035] p-4 text-sm font-bold text-slate-300"><input type="checkbox" checked={allowResubmission} onChange={(event) => setAllowResubmission(event.target.checked)} /> Allow the user to resubmit clearer documents</label><PrimaryButton disabled={rejectState.isLoading || !reason.trim()} className="mt-4 w-full" onClick={() => decide('reject')}>Save decision</PrimaryButton></Modal>
    </>
  )
}
