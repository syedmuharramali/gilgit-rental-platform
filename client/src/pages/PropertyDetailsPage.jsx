...
function PropertyDetailsPage() {
  ...
  if (error || !property) return <main className="grid min-h-[70vh] place-items-center bg-[#070b14] px-5 py-16 text-center text-white"><div><p className="text-2xl font-black">{t('details.notFound')}</p><p className="mt-2 text-sm text-slate-500">{t('details.notFoundText')}</p><Link to="/properties" className="mt-5 inline-flex rounded-full bg-gradient-to-r from-cyan-300 via-blue-400 to-violet-500 px-5 py-3 text-sm font-black text-[#07101e]">{t('details.back')}</Link></div></main>
  ...
  const messageOwner = async () => {
    try {
      ...
      toast.success(t('details.toastConversation')); navigate('/dashboard/messages', { state: { conversationId } })
    } catch (requestError) { toast.error(errorMessage(requestError)) }
  }

  const submitReport = async () => {
    try { await createReport({ targetType: 'property', targetId: id, ...report }).unwrap(); toast.success(t('details.toastReport')); setModal(null) }
    catch (requestError) { toast.error(errorMessage(requestError)) }
  }
  ...
  return (
    <main className="relative overflow-hidden bg-[#070b14] pb-24 text-white">
      ...
      <section className="relative z-10 px-4 py-4 sm:px-8 sm:py-5 lg:px-10">
        <div className="mx-auto max-w-[1440px]"><Link to="/properties" className="inline-flex items-center gap-2 text-sm font-bold text-slate-400 transition hover:text-white"><ChevronLeft className="h-4 w-4" /> {t('details.back')}</Link></div>
      </section>
        ...
        <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="mx-auto max-w-[1440px] overflow-hidden rounded-[26px] border border-white/[0.08] bg-white/[0.03] p-2 shadow-[0_35px_110px_rgba(0,0,0,.32)] sm:rounded-[32px] sm:p-3">
          <div className="grid gap-2 lg:h-[540px] lg:grid-cols-[1.28fr_.72fr] xl:h-[580px]">
            <button
              ...
              aria-label={galleryImages.length > 1 ? t('details.openGallery', { count: galleryImages.length }) : t('details.viewPhotoSingle')}
            >
              {displayedImage?.url ? <img src={displayedImage.url} alt={displayedImage.alt || property.title} className="h-full w-full object-cover transition duration-700 group-hover:scale-[1.015]" /> : <div className="grid h-full place-items-center bg-[radial-gradient(circle_at_30%_20%,rgba(56,189,248,.18),transparent_34%),linear-gradient(145deg,#111827,#0b1220)] font-bold text-white/60">{t('details.noImage')}</div>}
              <div className="absolute inset-0 bg-gradient-to-t from-[#040711]/65 via-transparent to-[#040711]/10" />
              {galleryImages.length > 1 && <span className="absolute right-3 top-3 inline-flex items-center gap-2 rounded-full border border-white/15 bg-[#070b14]/75 px-3 py-2 text-[10px] font-black uppercase tracking-[.1em] text-white shadow-lg backdrop-blur-xl sm:right-4 sm:top-4"><Images className="h-3.5 w-3.5 text-cyan-300" /> {t('details.photos', { count: galleryImages.length })}</span>}
              <div className="absolute bottom-4 left-4 flex flex-wrap gap-2 sm:bottom-5 sm:left-5"><span className="rounded-full border border-white/15 bg-[#070b14]/75 px-3 py-2 text-[10px] font-black uppercase tracking-[.12em] backdrop-blur-2xl">{pretty(property.propertyType)}</span><span className="inline-flex items-center gap-1.5 rounded-full border border-white/15 bg-[#070b14]/75 px-3 py-2 text-[10px] font-black uppercase tracking-[.12em] backdrop-blur-2xl"><ShieldCheck className="h-3.5 w-3.5 text-cyan-300" /> {t('details.verified')}</span>{isReserved && <span className="rounded-full border border-violet-300/25 bg-violet-400/20 px-3 py-2 text-[10px] font-black uppercase tracking-[.12em] text-violet-100 backdrop-blur-2xl">{t('details.reserved')}</span>}</div>
            </button>
              ...
              {secondary.map((image, index) => {
                ...
                return (
                  <button
                    ...
                    aria-label={t('details.openPhotoOf', { index: actualIndex + 1, total: galleryImages.length })}
                  >
                    ...
                    {index === 3 && hiddenCount > 0 && <span className="absolute inset-0 grid place-items-center bg-[#050816]/62 text-sm font-black backdrop-blur-[2px]">{t('details.morePhotos', { count: hiddenCount })}</span>}
                  </button>
                )
              })}
            </div>
            ...
            <div className="mt-2 flex gap-2 overflow-x-auto pb-1 lg:hidden [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
              {galleryImages.map((image, index) => (
                <button
                  ...
                  aria-label={t('details.showPhotoOf', { index: index + 1, total: galleryImages.length })}
          ...
          <div>
            <div className="flex flex-col gap-5 border-b border-white/[0.08] pb-8 sm:flex-row sm:items-start sm:justify-between">
              <div className="min-w-0">
                <p className="text-[10px] font-black uppercase tracking-[.22em] text-cyan-300">{t('details.eyebrow')}</p>
      ...
      {galleryIndex != null && activeGalleryImage && (
        <div className="fixed inset-0 z-[100] flex flex-col bg-[#03050b]/96 p-3 backdrop-blur-xl sm:p-5" role="dialog" aria-modal="true" aria-label={t('details.photoGallery')}>
          <div className="mx-auto flex w-full max-w-[1500px] items-center justify-between gap-4 pb-3">
            <div>
              <p className="text-xs font-black uppercase tracking-[.16em] text-cyan-300">{t('details.propertyPhotos')}</p>
              <p className="mt-1 text-sm font-bold text-slate-400">{t('details.photoPosition', { current: galleryIndex + 1, total: galleryImages.length })}</p>
            </div>
            <button type="button" onClick={() => setGalleryIndex(null)} className="grid h-11 w-11 place-items-center rounded-full border border-white/10 bg-white/[0.06] text-white transition hover:bg-white/[0.1]" aria-label={t('details.closeGallery')}><X className="h-5 w-5" /></button>
          </div>
          ...
          <div className="relative mx-auto flex min-h-0 w-full max-w-[1500px] flex-1 items-center justify-center overflow-hidden rounded-[24px] border border-white/[0.08] bg-black/30 sm:rounded-[30px]">
            ...
            {galleryImages.length > 1 && <><button type="button" onClick={previousGalleryImage} className="absolute left-2 grid h-11 w-11 place-items-center rounded-full border border-white/15 bg-[#070b14]/75 text-white shadow-xl backdrop-blur-xl transition hover:bg-[#070b14] sm:left-4 sm:h-12 sm:w-12" aria-label={t('details.previousPhoto')}><ChevronLeft className="h-5 w-5" /></button><button type="button" onClick={nextGalleryImage} className="absolute right-2 grid h-11 w-11 place-items-center rounded-full border border-white/15 bg-[#070b14]/75 text-white shadow-xl backdrop-blur-xl transition hover:bg-[#070b14] sm:right-4 sm:h-12 sm:w-12" aria-label={t('details.nextPhoto')}><ChevronRight className="h-5 w-5" /></button></>}
          </div>

          {galleryImages.length > 1 && <div className="mx-auto mt-3 flex w-full max-w-[1500px] gap-2 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">{galleryImages.map((image, index) => <button key={image.id} type="button" onClick={() => setGalleryIndex(index)} aria-label={t('details.viewPhoto', { index: index + 1 })} aria-pressed={galleryIndex === index} className={`h-16 w-20 shrink-0 overflow-hidden rounded-xl border transition sm:h-20 sm:w-28 ${galleryIndex === index ? 'border-cyan-300 ring-2 ring-cyan-300/15' : 'border-white/10 opacity-55 hover:opacity-100'}`}><img src={image.url} alt="" className="h-full w-full object-cover" /></button>)}</div>}
        </div>
      )}

      <Modal open={modal === 'apply'} onClose={() => setModal(null)} title={t('details.applyModalTitle')}>
        <div className="space-y-5">
          <div className="rounded-2xl border border-cyan-300/12 bg-cyan-300/[0.045] p-4">
            <p className="text-sm font-black text-cyan-100">{t('details.applyIntroTitle')}</p>
          ...
          <label className="block">
            ...
            <TextArea maxLength={1000} value={application.message} onChange={(event) => setApplication((current) => ({ ...current, message: event.target.value }))} placeholder={t('details.messagePlaceholder')} />
            <span className="mt-2 block text-[11px] leading-5 text-slate-500">{t('details.messageHint')}</span>
          </label>
          ...
          {application.applicationType === 'group' && (
            <div className="space-y-3 rounded-2xl border border-white/[0.08] bg-white/[0.025] p-4">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <p className="text-sm font-black">{t('details.roommates')}</p>
                  <p className="mt-1 text-[11px] leading-5 text-slate-500">{t('details.roommatesHint')}</p>
                </div>
                <SecondaryButton type="button" onClick={addRoommate}><Plus className="h-4 w-4" /> {t('details.addRoommate')}</SecondaryButton>
              </div>
              {application.roommates.map((roommate, index) => (
                <div key={index} className="rounded-2xl border border-white/10 bg-[#0b111e] p-3">
                  <p className="mb-3 text-xs font-black text-slate-300">{t('details.roommateNumber', { index: index + 1 })}</p>
                  <div className="grid gap-3 sm:grid-cols-2">
                    <label className="block"><span className="mb-1.5 block text-[11px] font-bold text-slate-400">{t('details.fullName')}</span><TextInput value={roommate.name} onChange={(event) => updateRoommate(index, 'name', event.target.value)} placeholder={t('details.fullName')} /></label>
                    <label className="block"><span className="mb-1.5 block text-[11px] font-bold text-slate-400">{t('details.emailLabel')}</span><TextInput type="email" value={roommate.email} onChange={(event) => updateRoommate(index, 'email', event.target.value)} placeholder={t('details.emailPlaceholder')} /></label>
                  </div>
                  <div className="mt-3 flex items-end gap-2">
                    <label className="block flex-1"><span className="mb-1.5 block text-[11px] font-bold text-slate-400">{t('details.phone')} <span className="font-semibold text-slate-600">{t('details.optional')}</span></span><TextInput value={roommate.phone} onChange={(event) => updateRoommate(index, 'phone', event.target.value)} placeholder={t('details.phonePlaceholder')} /></label>
                    <button type="button" onClick={() => removeRoommate(index)} className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-rose-400/10 text-rose-300" aria-label={t('details.removeRoommate', { index: index + 1 })}><Trash2 className="h-4 w-4" /></button>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="rounded-2xl border border-white/[0.07] bg-white/[0.025] p-4 text-xs leading-5 text-slate-500">{t('details.applyDisclaimer')}</div>

          <PrimaryButton disabled={applicationState.isLoading || (application.applicationType === 'group' && application.roommates.length === 0)} className="w-full" onClick={submitApplication}>{applicationState.isLoading ? t('details.submitting') : t('details.submitApplication')}</PrimaryButton>
        </div>
      </Modal>
