...
export default function VerifyEmailPage() {
  ...
  return (
    <AuthLayout
      ...
      title={state === 'done' ? 'Your email is confirmed.' : 'Confirming your email.'}
      subtitle="One click keeps your account and your rental conversations secure."
    >
      {state === 'checking' && (
      ...
      {state === 'done' && (
        <div className="rounded-[20px] border border-cyan-300/25 bg-cyan-300/[0.07] p-5">
          ...
          <p className="mt-3 text-sm font-black text-cyan-100">Confirmed. You are signed in.</p>
          <p className="mt-1 text-xs leading-6 text-white/60">Taking you to your dashboard…</p>
          <Link to="/dashboard" className="mt-4 inline-flex min-h-11 items-center rounded-xl bg-gradient-to-r from-cyan-300 via-blue-400 to-violet-500 px-4 text-xs font-black text-[#07101e]">Go now</Link>
        </div>
      )}
        ...
        <div className="space-y-4">
          <div className="rounded-[20px] border border-rose-400/25 bg-rose-400/[0.07] p-5">
            ...
            <p className="mt-3 text-sm font-black text-rose-100">
              {state === 'missing' ? 'This link is incomplete' : 'This link did not work'}
            </p>
            <p className="mt-1 text-xs leading-6 text-white/60">
              {state === 'missing'
                ? 'Open the link straight from the email, or copy the whole address including the part after the question mark.'
                : message}
            </p>
          </div>
          ...
          <VerificationNotice
            ...
            text={pendingEmail
              ? 'We can send another confirmation link to this address.'
              : 'Sign in with your email and password and we will offer you a new link.'}
          />

          <p className="text-center text-sm text-white/38">
            <Link className="font-black text-cyan-200 hover:text-white" to="/login">Back to sign in</Link>
          </p>
        </div>
      )}
    </AuthLayout>
